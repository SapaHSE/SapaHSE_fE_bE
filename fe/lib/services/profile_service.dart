...
class ProfileService {
  ...
  static Future<ProfileResult> getProfile() async {
    ...
    if (!response.success) {
      return ProfileResult.error(response.errorMessage ?? 'Gagal memuat profil.');
    }
  ...
  static Future<ProfileResult> updateProfile({
    ...
    if (!response.success) {
      return ProfileResult.error(response.errorMessage ?? 'Gagal menyimpan profil.');
    }
  ...
  static Future<SimpleResult> changePassword({
    ...
    if (!response.success) {
      return SimpleResult.error(response.errorMessage ?? 'Gagal mengubah password.');
    }

    return SimpleResult.success(response.data['message'] ?? 'Password berhasil diubah.');
  }
}
