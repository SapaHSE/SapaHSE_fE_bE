...
class _MedicalContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final history = [
      _MedicalHistory(
        id: 'mh1', title: 'Medical Check-Up Tahunan 2026',
        date: '10 Januari 2026', doctor: 'dr. Andi Wijaya, Sp.OK',
        facility: 'Klinik Pratama BBE', result: 'Fit to Work',
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
              ...
              children: List.generate(records.length, (i) {
                ...
                return Column(children: [
                  Padding(
                    ...
                    child: Row(children: [
                      Expanded(flex: 2, child: Text(r['label']!, style: const TextStyle(fontSize: 13, color: Colors.black54))),
                      const Text(': ', style: TextStyle(fontSize: 13, color: Colors.black54)),
                      Expanded(flex: 3, child: Text(r['value']!, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.black87))),
                    ]),
                  ),
          ...
          Row(children: [
            const Text('Riwayat Pemeriksaan', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
            const Spacer(),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(color: const Color(0xFF1A56C4).withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
              child: Text('${history.length} riwayat', style: const TextStyle(fontSize: 11, color: Color(0xFF1A56C4), fontWeight: FontWeight.bold)),
            ),
          ]),
...
class _MedicalHistory {
  ...
  const _MedicalHistory({
    ...
    required this.doctor, required this.facility, required this.result,
    required this.resultColor, required this.notes, required this.items,
  });
}
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            width: 44, height: 44,
            decoration: BoxDecoration(color: history.resultColor.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
            child: Icon(Icons.medical_services_outlined, color: history.resultColor, size: 22),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(history.title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 3),
              Row(children: [
                const Icon(Icons.calendar_today_outlined, size: 11, color: Colors.grey),
                const SizedBox(width: 4),
                Text(history.date, style: const TextStyle(fontSize: 11, color: Colors.grey)),
                const SizedBox(width: 10),
                const Icon(Icons.local_hospital_outlined, size: 11, color: Colors.grey),
                const SizedBox(width: 4),
                Expanded(child: Text(history.facility, style: const TextStyle(fontSize: 11, color: Colors.grey), maxLines: 1, overflow: TextOverflow.ellipsis)),
              ]),
            ]),
          ),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Container(
              ...
              decoration: BoxDecoration(color: history.resultColor, borderRadius: BorderRadius.circular(20)),
              child: Text(history.result, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w600)),
            ),
            const SizedBox(height: 6),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Detail Pemeriksaan', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 16)),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF1A56C4), borderRadius: BorderRadius.circular(16)),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(
                  width: 40, height: 40,
                ...
                const SizedBox(width: 12),
                Expanded(child: Text(history.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15))),
              ]),
              const SizedBox(height: 14),
              ...
              const SizedBox(height: 14),
              Row(children: [
                const Text('Hasil:', style: TextStyle(color: Colors.white70, fontSize: 13)),
                const SizedBox(width: 8),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5),
                  decoration: BoxDecoration(color: history.resultColor, borderRadius: BorderRadius.circular(20)),
                  child: Text(history.result, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                ),
              ]),
            ]),
          ),
          ...
          _SectionCard(
            ...
            trailing: Text('$done/$total selesai', style: const TextStyle(fontSize: 12, color: Color(0xFF1A56C4), fontWeight: FontWeight.w600)),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation(done == total ? const Color(0xFF4CAF50) : const Color(0xFF1A56C4)),
                ),
              ),
              const SizedBox(height: 14),
              ...history.items.map((item) => Padding(
                ...
                child: Row(children: [
                  Container(
                    width: 22, height: 22,
                    decoration: BoxDecoration(
                      color: item.done ? const Color(0xFF4CAF50) : Colors.grey.shade200,
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: Text(item.label, style: TextStyle(fontSize: 13, color: item.done ? Colors.black87 : Colors.grey, decoration: item.done ? null : TextDecoration.lineThrough))),
                  Text(item.done ? 'Selesai' : 'Tidak', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: item.done ? const Color(0xFF4CAF50) : Colors.grey)),
                ]),
              )),
            ]),
          ),
          ...
          _SectionCard(
            ...
            child: Text(history.notes, style: const TextStyle(fontSize: 13, color: Colors.black87, height: 1.6)),
          ),
          const SizedBox(height: 80),
        ]),
      ),
...
class _HeaderRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    Expanded(child: Text(value, style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
  ]);
}
  ...
  void _showDeleteAccountDialog(BuildContext context) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Tindakan ini tidak dapat dibatalkan. Seluruh data Anda akan dihapus secara permanen.\n\nApakah Anda yakin?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Batal')),
          ElevatedButton(
...
class _DropdownRow extends StatelessWidget {
  ...
  const _DropdownRow({required this.icon, required this.iconColor, required this.label, required this.subtitle, required this.value, required this.items, required this.onChanged});
...
class _MenuRow extends StatelessWidget {
  ...
  const _MenuRow({required this.icon, required this.iconColor, required this.label, this.labelColor, required this.onTap});
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: Padding(
      ...
      child: Row(children: [
        Container(width: 36, height: 36, decoration: BoxDecoration(color: iconColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: iconColor, size: 20)),
        const SizedBox(width: 12),
        Expanded(child: Text(label, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: labelColor ?? Colors.black87))),
